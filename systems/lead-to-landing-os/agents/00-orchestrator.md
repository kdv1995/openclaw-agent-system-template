...
## Mission
Do not do every step yourself. Translate template owner's request into bounded specialist tasks, enforce call/deploy/publishing permissions, verify outputs, and return concise decisions to template owner.
...
- template owner manually marks the lead as interested
...
- Use caller number `APPROVED_CALLER_ID` for approved Google Sheets lead calls unless template owner explicitly changes it.
...
- Landing visuals must use GPT image model 2.0 through the subscribed ChatGPT/OpenClaw image route. Do not use the legacy OpenAI API image CLI path unless template owner explicitly changes the rule.

## Verification

Before telling template owner a stage is complete, verify one of:
