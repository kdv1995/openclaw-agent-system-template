...
def _resolve_telegram_target(args: argparse.Namespace) -> str:
    target = getattr(args, "telegram_target", None) or os.getenv("OPENCLAW_TELEGRAM_TARGET")
    if not target:
        _die(
            "Telegram target is missing. Use --telegram-target <chat_id_or_username> "
            "or set OPENCLAW_TELEGRAM_TARGET."
        )
    return str(target)


def _send_file_to_telegram(
    *,
    path: Path,
    target: str,
    ...
    if dry_run:
        print(json.dumps({"telegram_dispatch": cmd}, ensure_ascii=False))
        return

    completed = subprocess.run(cmd, capture_output=True, text=True)
    if completed.returncode != 0:
        details = completed.stderr.strip() or completed.stdout.strip() or "unknown error"
        _die(f"Failed to send {path} to Telegram via OpenClaw: {details}")
    print(f"Sent to Telegram: {path}")


def _maybe_send_outputs_to_telegram(
    args: argparse.Namespace,
    outputs: List[Path],
) -> None:
    if not getattr(args, "send_telegram", False):
        return
    target = _resolve_telegram_target(args)
    message = getattr(args, "telegram_message", None)
    force_document = bool(getattr(args, "telegram_force_document", False))
    for path in outputs:
...
async def _run_generate_batch(args: argparse.Namespace) -> int:
            ...
            _print_request(
                {
                    ...
                    "telegram_send": bool(args.send_telegram),
                    "telegram_target": (
                        args.telegram_target
                        if args.telegram_target
                        else os.getenv("OPENCLAW_TELEGRAM_TARGET")
                    ),
                    **job_payload,
                }
            )
    ...
    async def run_job(i: int, job: Dict[str, Any]) -> Tuple[int, Optional[str]]:
        ...
        try:
            ...
            _maybe_send_outputs_to_telegram(args, outputs)
            return i, None
        except Exception as exc:
            any_failed = True
...
def _generate(args: argparse.Namespace) -> None:
    ...
    if args.dry_run:
        ...
        _maybe_send_outputs_to_telegram(args, output_paths)
        return
...
def _add_shared_args(parser: argparse.ArgumentParser) -> None:
    ...
    # Optional dispatch to Telegram via OpenClaw after files are written.
    parser.add_argument("--send-telegram", action="store_true")
    parser.add_argument("--telegram-target")
    parser.add_argument("--telegram-message")
    parser.add_argument("--telegram-force-document", action="store_true")
