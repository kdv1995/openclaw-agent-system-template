...
8. If requested, dispatch generated files to Telegram using `--send-telegram` (via OpenClaw channel delivery).
...
- For auto-delivery to Telegram, set `OPENCLAW_TELEGRAM_TARGET` (or pass `--telegram-target` each run).
