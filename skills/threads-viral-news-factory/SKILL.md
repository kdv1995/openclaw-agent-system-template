...
Use for template owner's Threads workflow.

## Routing

- Source: X/Twitter first.
- Destination: Threads `your_profile` through Postiz, with direct Threads API fallback only when explicitly approved or when Postiz is unavailable and template owner has asked to publish now.
- Content type: viral/fresh news or strong insight from X/Twitter.
- No images by default. Image posts route to Facebook unless template owner says otherwise.
...
4. Validate every Threads item before Postiz:
   ...
   - single-post format is allowed only for tiny updates, replies, or when template owner explicitly asks for "коротко"
   - mini-thread can exceed 7 posts only when template owner asks for depth
...
6. If Postiz fails and template owner explicitly approves fallback, use direct Threads API via `scripts/threads_publish_direct_fallback.mjs`. This requires `THREADS_ACCESS_TOKEN` and `THREADS_USER_ID` in `<openclaw-home>/.env`.
...
- Never paste or store access tokens from chat. If a token was sent in chat, ask template owner to rotate it and put the replacement in `.env`.
