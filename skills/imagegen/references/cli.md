...
- Do not modify `scripts/image_gen.py` unless the user explicitly asks for a CLI behavior change.
...
Generate and auto-send to Telegram via OpenClaw:

```
python "$IMAGE_GEN" generate \
  --prompt "Banana astronaut floating in deep space, cinematic lighting" \
  --out "$HOME/.openclaw/workspace/banana_space.png" \
  --send-telegram \
  --telegram-target "-5197331552" \
  --telegram-message "Generated image"
```

Notes:
- `--send-telegram` calls `openclaw message send --channel telegram --media ...` for each produced image.
- You can set `OPENCLAW_TELEGRAM_TARGET` once and omit `--telegram-target`.
- Use `--telegram-force-document` to avoid Telegram image recompression.
